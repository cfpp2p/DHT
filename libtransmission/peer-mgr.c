...
addStrike( Torrent * t, tr_peer * peer )
{
    ...
    if( ++peer->strikes >= MAX_BAD_PIECES_PER_PEER )
    {
        ...
        bl_tordbg( t, "IP %s banned for sending too many bad pieces.", tr_atomAddrStr( atom ) );
    }
}
                   ...
                   void          * vmanager )
{
    ...
    else /* looking good */
    {
        ...
        if( atom->flags2 & MYFLAG_BANNED )
        {
            ...
            bl_tordbg( t, "banned -bad pieces- peer %s tried to reconnect", tr_atomAddrStr( atom ) );
        }
                       ...
                       struct UTPSocket * utp_socket )
{
    ...
    {
        tr_dbg( "-Incoming- Blocklisted IP address \"%s\" tried to connect to us", tr_address_to_string( addr ) );
        if(socket >= 0)
...
isAtomInteresting( const tr_torrent * tor, struct peer_atom * atom )
{
    ...
    {
    tr_tordbg( tor, "Blocklisted IP %s dropped as atom peer interesting", tr_address_to_string( &atom->addr ) );
    return false;
    }

    if( atom->flags2 & MYFLAG_BANNED )
    {
    tr_tordbg( tor, "-too many bad pieces- IP %s dropped as atom peer interesting", tr_address_to_string( &atom->addr ) );
    return false;
    }
...
isPeerCandidate( const tr_torrent * tor, struct peer_atom * atom, const time_t now )
{
    ...
    if( isAtomBlocklisted( tor->session, atom ) )
    {
    tr_tordbg( tor, "Blocklisted IP %s dropped as atom peer candidate", tr_address_to_string( &atom->addr ) );
    return false;
    }
    ...
    if( atom->flags2 & MYFLAG_BANNED )
    {
    tr_tordbg( tor, "-too many bad pieces- IP %s dropped as atom peer candidate", tr_address_to_string( &atom->addr ) );
    return false;
    }
